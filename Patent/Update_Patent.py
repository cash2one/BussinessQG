...
class Ia_Patent:
	...
	def get_info(self, first_info, cookies):
		...
		for i, single in enumerate(first_info):
			logging.info('now the start threading number is :%s '%i)
		...
		try:
			for key in info.keys():
				...
				if count == 0:
					...
					ia_patent_id = cursor.fetchall()[0][0]
					self.upadte_law_info(law_search_info, cursor, connect, ia_patent_id)
					self.update_same_to_db(same_info, cursor, connect, ia_patent_id)
		except Exception, e:
			remark_flag = 100000006
			print e
			logging.error("update error:%s" % e)
			# print e
		finally:
			if remark_flag < 100000006:
				remark_flag = total
			
			return remark_flag, total, insert_flag
	# 将法律信息采集更新到数据库中

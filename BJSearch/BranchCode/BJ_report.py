...
url = 'http://qyxy.baic.gov.cn/wapqyzb/wapqyzbAction!wapbsnd.dhtml?entId=20e38b8c45c6dea10145d9d35ad233b7'
...
class Report:
	...
	def get_all_report_info(self,href_list,cursor,connect,gs_basic_id,gs_py_id,cookies):
		total = len(href_list)
		insert = 0
		error = 0
		for key in href_list.keys():
			...
			if int(count) > 0:
				...
				mark = self.get_single_report_info(href, year, cursor, connect, cookies, gs_basic_id,gs_py_id)
				if mark ==1:
					insert +=1
				else:
					error+=1
		return total,insert,error
	def get_single_report_info(self,url,year,cursor,connect,cookies,gs_basic_id,gs_py_id):
		...
		if mark ==1:
			if "basic" in branch_list.keys():
				...
				if flag ==1:
					gs_report_id,remark=BJ_report_basic.update_to_db(gs_basic_id, info, year, cursor, connect,province)
					if remark ==1:
						...
						if "run" in branch_list.keys():
							...
							if flag == 1 and len(info)>0:
								remark = BJ_report_run.update_run_info(year,gs_report_id,gs_basic_id, cursor,connect, info, province)
...
def main(gs_search_id,gs_basic_id,url):
	...
	try:
		...
		connect.close()
		cursor.close()
		print "report:"+ str(flag) + '||' + str(total) + '||' + str(insert) + '||' + str(error)

...
def name(url,cookies,headers):
	...
	if status_code ==200 :
		...
		if u"资金数额" in content:
			...
			amount = None
		info[0] = [name,code,ccode,tel,postcode,email,status,address,if_website,if_invest,employee,runner,amount]
	else:
		flag = 100000004
	return info,flag
# 用于处理dd标签中的内容
...
def update_to_db(gs_basic_id,info,year,cursor,connect,province):
	try:
		...
		gs_basic_id, year, province, name, uuid, tel, address, email, postcode, status, employee, code, ccode,  if_invest,if_website, runner,amount, updated_time, updated_time))

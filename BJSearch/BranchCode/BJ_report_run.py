		...
		if len(list)>0:
			...
			if u"负债总额" in content:
				...
				if u"不公示" in content:
					...
					debt = deal_html_code.match_finger(debt)
					if_debt = 1
			else:
				debt = None
				if_debt = 0
			...
			info = [asset,if_asset,benifit,if_benifit,profit,if_profit,main_income,if_main,income,if_income,net_income,if_net,tax,if_tax,debt,if_debt,loan, if_loan, subsidy, if_subsidy]
		else:
			logging.info("无资产信息")
	else:
		flag = 100000004
...
def update_run_info(year, gs_report_id, gs_basic_id, cursor, connect,  runinfo,province):
	...
	try:
		...
		gs_report_id, gs_basic_id, province, asset, if_asset, benifit, if_benifit, income, if_income, profit, if_profit,
		main_income, if_main, net_income, if_net, tax, if_tax, debt, if_debt, loan, if_loan, subsidy, if_subsidy, uuid,

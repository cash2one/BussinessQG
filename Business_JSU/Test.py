...
def get_cookies():
	...
	while i > 0:
		try:
			request = session.get("http://www.gsxt.gov.cn/index.html", headers=config.headersfirst, proxies=proxies,
								  timeout=5)
			# print request
...
def break_password(cookies):
	...
	try:
		result = session.get(url, cookies=cookies, headers=config.headersfirst, proxies=proxies)
...
def get_url_info(challenge, validate, string, cookies):
	...
	result = session.post(url, search_text, cookies=cookies, headers=config.headers, proxies=proxies)
		...
		if history_name != None:
			...
			if count == 0:
				...
				cursor.execute(update_basic,
							   (gs_basic_exp_id, gs_basic_id, history_name, updated_time, gs_basic_exp_id))
				connect.commit()

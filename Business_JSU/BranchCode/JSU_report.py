...
class Report:
	def __init__(self, url, headers, org, id, seqid, regno,gs_basic_id,gs_py_id):
		self.url = url
	...
	def update_single_report_info(self,year,report_id,cursor,connect):
		...
		try:
			...
			if flag == 1:
				...
				JSU_report_assure.main(report_id, gs_report_id, cursor, connect, self.gs_basic_id,self.gs_py_id)
				JSU_report_invest.main(report_id, gs_report_id, cursor, connect, self.gs_basic_id,self.gs_py_id)
				JSU_report_permit.main(report_id, gs_report_id, cursor, connect, self.gs_basic_id,self.gs_py_id)
				JSU_report_schange.main(report_id, gs_report_id, cursor, connect, self.gs_basic_id,self.gs_py_id)
				JSU_report_share.main(report_id, gs_report_id, cursor, connect, self.gs_basic_id,self.org, self.id, self.seqid,self.regno,self.gs_py_id)
				JSU_report_web.main(report_id, gs_report_id, cursor, connect, self.gs_basic_id,self.gs_py_id)
				if int(year) ==2016:
...
def main(gs_basic_id, org, id, seqid, regno,gs_py_id):
	remark, total, insert = report_main(org, id, seqid, regno, gs_basic_id,gs_py_id)
	print "report:"+str(remark)+"||"+str(total)+"||"+str(insert)
	
# if __name__ == '__main__':
# 	print "The Program start time:", time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
# 	start = time.time()
# 	gs_basic_id = 1900000877
# 	main(gs_basic_id, org, id, seqid, regno)

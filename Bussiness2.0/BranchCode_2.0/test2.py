...
def get_cookies():
    ...
    while i > 0:
        try:
            request = session.get("http://www.gsxt.gov.cn/index.html", headers=config.headersfirst,proxies = config.proxies, timeout=5)
...
def get_url_info(challenge, validate,string,cookies):
    ...
    result = session.post(url, search_text, cookies=cookies, headers=config.headers,proxies = config.proxies)

        ...
        try:
            ...
            for key in info.keys():
                ...
                if int(count) == 0:
                    ...
                    row_count = cursor.execute(search_string,(gs_basic_id,user_id,unique_id,keyword,company,provin,code,code,legal_person,runner,responser,investor,dates,status,if_new,url,pagenumber,updated))
                    connect.commit()
    ...
    try:
        ...
        if int(pagenumber) ==1:
            ...
            if cookies == None:
                ...
                flag, insert_flag, update_flag = searchobject.insert_search(keyword, user_id, info, cursor, connect,pagenumber)
                ...
                info,total_page= searchobject.last_request(url,keyword,challenge_params, validate_params, cookies,pagenumber)
                flag, insert_flag, update_flag = searchobject.insert_search(keyword, user_id, info, cursor, connect,pagenumber)
                searchobject.printinfo(flag,insert_flag,update_flag,unique_id,cookies,validate_params,challenge_params,total_page,pagenumber)
    except Exception,e:
        flag = 100000005
        searchobject.printinfo(flag,insert_flag,update_flag,unique_id,cookies,validate,challenge,total_page,rspage)

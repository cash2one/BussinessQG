...
class Report:
    def update_all_info(self, gs_basic_id, year, url, cursor, connect):
        ...
        try:
            ...
            App_report_assure.Assure().update_to_db(report_id, gs_basic_id, cursor, connect, assureinfo, province)

            investinfo = App_report_invest.Invest().name(investinfo)
            App_report_invest.Invest().update_to_db(report_id, gs_basic_id,
            ...
            App_report_permit.Permit().update_to_db(report_id, gs_basic_id,
                                                    cursor, connect, permitinfo,
                                                    province)

            partyinfo = App_report_party.Party().name(partyinfo)
            App_report_party.Party().update_to_db(report_id, gs_basic_id,
                                                  cursor, connect, partyinfo,
                                                  province)

            schangeinfo = App_report_schange.Schange().name(schangeinfo)
            App_report_schange.Schange().update_to_db(report_id, gs_basic_id,
                                                      cursor, connect, schangeinfo,
                                                      province)

            shareinfo = App_report_share.Share().name(shareinfo)
            App_report_share.Share().update_to_db(report_id, gs_basic_id,
                                                  cursor, connect, shareinfo,
                                                  province)

            webinfo = App_report_web.Web().name(webinfo)
            App_report_web.Web().update_to_db(report_id, gs_basic_id,
...
def update_report_main(gs_basic_id, year, cursor, connect):
    reportobject = Report()
    count = cursor.execute(select_report, (gs_basic_id, year))
    if int(count) > 0:
        flag = 0
    else:
        flag = reportobject.update_all_info(gs_basic_id, year, url, cursor, connect)
    return flag
...
def main():
    ...
    try:
        ...
        flag = update_report_main(gs_basic_id,  year, cursor, connect)

...
insert_ps = 'insert into ps_basic(gs_basic_id,name,idcard,province,city,town,birthday,encode,created,updated) values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)'
...
class Share:
    ...
    def judge_certcode(self,name,code,cursor,connect,basic_id):
        ps = 0
        if len(code)== 15 or len(connect)==18:
            select_string = select_ps %name
            count = cursor.execute(select_string)
            if int(count)==1:
                for ps_basic_id,gs_basic_id in cursor.fechall():
                ...
                updated_time = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(time.time()))
                cursor.execute(insert_ps,(gs_basic_id,name,idcard,province,city,town,birthday,encode,updated_time,updated_time))
...
def main(gs_search_id,gs_basic_id,data):
    ...
    Judge_status().updaye_py(gs_basic_id,Share,"shareholder",data)

...
class Brand:
	def name(self,url):
		...
		if status_code ==200:
			...
			datalist = etree.tostring(dl).replace("<dl>",'').replace("</dl>",'').split('<dd style="border-top:1px dashed #ccc;">')
			...
			for key in info.keys():
				...
				if count == 0:
					...
					rows_count = cursor.execute(update_brand,
												(gs_brand_id, gs_basic_id, ia_name,ia_zch,ia_type,updated_time, gs_brand_id))

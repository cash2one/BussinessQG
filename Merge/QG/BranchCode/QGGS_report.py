...
class Report:
    ...
    def get_report_info(self, year,anCheId,province):
        ...
        try:
            ...
            records,total,insert_total,update_total = Get_BranchInfo().get_info(report_id, self.gs_basic_id, self.cursor, self.connect, url_list["web"],
                                                                   QGGS_report_web.Web, "reportweb%s" % year)
           
            records,total,insert_total,update_total = Get_BranchInfo().get_info(report_id, self.gs_basic_id, self.cursor, self.connect, url_list["invest"],
                                          QGGS_report_invest.Invest, "reportinvest%s" % year)
           
            records,total,insert_total,update_total = Get_BranchInfo().get_info(report_id, self.gs_basic_id, self.cursor, self.connect, url_list["share"],
                                          QGGS_report_shareholder.Share, "reportshare%s" % year)
           
            records,total,insert_total,update_total = Get_BranchInfo().get_info(report_id, self.gs_basic_id, self.cursor, self.connect, url_list["schange"],
            ...
            if count ==0:
                ...
                if counts ==0:
                    ...
                    row_count = self.cursor.execute(update_frun, (
                        gs_report_id, self.gs_basic_id, province, uuid, income, if_income, profit, if_profit, tax, if_tax, loan,
...
def main(gs_basic_id,url):
    ...
    try:
        ...
        logging.info("unknown report error:%s"%e)
    finally:
        cursor.close()
        connect.close()
        
        print "report:" + str(flag)+"||"+str(total)+"||"+str(insert_total)+"||"+str(update_total)

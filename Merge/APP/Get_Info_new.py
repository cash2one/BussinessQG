...
select_string1 = 'select gs_basic_id from gs_basic where code = %s or ccode = %s or code = %s'
...
def insert_to_basic(information,cursor,connect):
	...
	cursor.execute(insert_string, (('S',id, province, entName, code, ccode, legal_person, dates, status, updated)))
	gs_basic_id = connect.insert_id()
	connect.commit()
	return gs_basic_id
def update_info(count,cursor,connect,second_url,province,information,gs_new_id):
	if int(count) == 1:
					...
					if name == '' and code == '' and code == '':
						...
						if flag == 1:
							...
							if code!=None and ccode!=None:
								count = cursor.execute(select_string,(code, ccode, code))
								update_info(count, cursor, connect, second_url, province, information, gs_new_id)
			
							elif code!=None:
								count = cursor.execute(select_string,(code,code))
								update_info(count, cursor, connect, second_url, province, information, gs_new_id)
							elif ccode!=None:
								count = cursor.execute(ccode,ccode)
								update_info(count, cursor, connect, second_url, province, information, gs_new_id)
						else:

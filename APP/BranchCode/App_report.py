...
class Report:
	...
	def update_all_info(self, gs_basic_id, year, url, cursor, connect):
		...
		try:
			...
			flag, total, insert_total, update_total = App_report_assure.Assure().update_to_db(report_id, gs_basic_id,
																							  cursor, connect,
																							  assureinfo, province)
			...
			if len(labinfo) > 0:
				...
				if len(labinfo) > 0:
					App_report_lab.Lab().update_to_db(report_id, gs_basic_id, self.gs_py_id, cursor, connect, labinfo,
													  province)
...
def update_report_main(gs_basic_id, gs_py_id, year, cursor, connect):
...
def main():
	...
	try:
		...
		flag = update_report_main(gs_basic_id, gs_py_id, year, cursor, connect)

...
web_string = 'insert into gs_report_web(gs_basic_id,id,gs_report_id,name,types,website,updated) values(%s,%s,%s,%s,%s,%s,%s)'
select_web = 'select gs_report_web_id from gs_report_web where gs_report_id = %s and name = %s and website = %s '
update_web = 'update gs_report_web set types = %s,updated = %s where gs_report_web_id = %s'
...
def name(data):
    ...
    for i in xrange(len(data)):
        ...
        information[i] = [types, name, website]
    return information
            ...
            try:
                ...
                if count == 0:
                    ...
                    flag = cursor.execute(web_string,
                                          (gs_basic_id, id, gs_report_id, name, types, website, updated_time))
                    ...
                    updated_time = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(time.time()))
                    flag = cursor.execute(update_web, (types, updated_time, gs_report_web_id))
                    connect.commit()
                    update_flag += flag
            except Exception, e:
                print e

...
def update_to_db(gs_basic_id, cursor, connect, information):
    ...
    for key in information.keys():
        ...
        try:
            ...
            update_except = 'update gs_except set types = %s ,in_reason = %s,out_reason = %s ,out_date,gov_dept = %s ,updated = %s where gs_except_id = %s'
            ...
            if count == 0:
                ...
                types, in_reason, in_date, out_reason, out_date, gov_dept, updated_time, gs_except_id))
                update_flag += rows_count
                connect.commit()
        except Exception, e:
            # print "except error:", e
            logging.error("except error:" % e)
